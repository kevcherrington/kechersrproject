...
public class EventsDbAdapter {
	...
	/*
	 * Database creation sql statement Sqlite3 ints can be upto 8 bytes in size.
	 * equivalent to a 64bit int. or a Java long...
	 */
	...
	/*
	 * Create a new event using the title provided. If the event is successfully
	 * created return the new rowId for that event, otherwise return a -1 to
	 * indicate failure.
	 */
	public long createEvent(String title, int runTimeHour, int runTimeMin,
			int sun, int mon, int tues, int wed, int thur, int fri, int sat,
			String mode, int vol, int vibe, long nextRun, long lastRun) {
	...
	public Cursor fetchAllEvents() {
		...
		return mDb.query(DATABASE_TABLE, new String[] { KEY_ROWID, KEY_TITLE,
				...
				KEY_VOL, KEY_VIBRATE, KEY_LAST_RUN, KEY_NEXT_RUN }, null, null,
				null, null, null);
	}
	...
	public Cursor fetchEvent(long rowId) throws SQLException {
		...
		Cursor mCursor = mDb.query(true, DATABASE_TABLE, new String[] {
				KEY_ROWID, KEY_TITLE, KEY_RUN_TIME_HOUR, KEY_RUN_TIME_MINUTE,
				KEY_SUN, KEY_MON, KEY_TUES, KEY_WED, KEY_THUR, KEY_FRI,
				KEY_SAT, KEY_MODE, KEY_VOL, KEY_VIBRATE, KEY_LAST_RUN,
				KEY_NEXT_RUN }, KEY_ROWID + "=" + rowId, null, null, null,
				null, null);
		if (mCursor != null) {
			mCursor.moveToFirst();
		}
		Log.v(TAG, "Cursor size is " + mCursor.getCount() + " Requested Row = "
				+ rowId);
		return mCursor;
	 ...
	 * passed in
	 */
	public boolean updateEvent(long rowId, String title, int runTimeHour,
			int runTimeMin, int sun, int mon, int tues, int wed, int thur,
			int fri, int sat, String mode, int vol, int vibe, long lastRun,
			long nextRun) {
	...
	/*
	 * Update the run times for the given row Id. Used primarily for scheduling and 
	 * rescheduling events.
	 */
	public Cursor updateEventRunTimes(Long rowId, Long lastRun, Long nextRun) {
		return mDb.rawQuery(("UPDATE " + DATABASE_TABLE + " SET "
				+ KEY_LAST_RUN + "=?, " + KEY_NEXT_RUN + "=? WHERE "
				+ KEY_ROWID + " =?"), new String[] { lastRun.toString(),
				nextRun.toString(), rowId.toString() });
	}
}
